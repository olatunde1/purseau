...
const OrderDetails = () => {
  ...
  return (
    <div className="container-sidebar flex justify-center font-custom">
      <aside className="w-[382px] p-4 h-[648px] border-r personal-information-sidebar"
          style={{
            boxShadow: `
      ...
      >
        <nav className="space-y-0">
          ...
          <Button className=" w-full bg-[#FFF4F0] text-[#E94E30] hover:bg-[#E94E30] hover:text-white py-8 
             track-order">Logout</Button>
        </nav>
      </aside>

      {/* Main Content */}
      <main className="ml-8 w-[878px pb-]">
          ...
          <CardContent className="space-y-6 mx-10 my-6">
            ...
            <div className="flex justify-between px-pb-4 ">
              <div>
                ...
                <p className={`text-sm mb-5 mt-2 font-medium px-3 py-1 rounded-2xl inline-block ${getStatusStyle(order.status)}`}>
                {order.status}
                </p>
              ...
              <div className="flex py-5">
                <div className="space-y-4">
                  {order.items.map((item) => (
                    <div key={item.id} className="flex rounded-md bg-[#F2F2F7] justify-between p-5">
                      <img src={item.image} alt={item.name} className="w-[130px] h-[130px] object-cover rounded-md" />
                      <div className="flex-1 mr-0 sm:mr-[212px] ml-6">
                        <p className="font-semibold text-base">{item.name}</p>
                        <p className="font-medium text-gray-600 pt-3">Quantity: <strong className="text-black">{item.quantity}</strong></p>
                        <p className="font-bold text-base text-black pt-3">₦ {item.price.toLocaleString()}</p>
                      </div>
                      <Button className="text-white bg-[#F2542D] hover:bg-[#E94E30] hover:text-white"
                        style={{
                            boxShadow: `
            ...
            <div className="pb-10">
              ...
              <div>
                <p className="text-[#5B5B5B]"><strong className="pr-4 text-black">Delivery Method: </strong> {order.deliveryInfo.method}</p>
                <p className="text-[#5B5B5B]"><strong className="pr-4 text-black">Shipping Address: </strong> {order.deliveryInfo.address}</p>
              </div>
            </div>
